...
export default function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const requestedSystems = Object.keys(requested).filter(
        (k) => requested[k]
      );
    ...
    <div className="min-h-screen bg-gradient-to-r from-slate-50 to-slate-100 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-10 ring-1 ring-slate-200">
        ...
        <p className="text-center text-slate-500 text-sm mt-2 mb-6">
          Register to access the database management systems
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="mt-2 block w-full rounded-lg border border-slate-200 px-4 py-2 text-black/80 placeholder:text-black/20"
          ...
          <fieldset className="mt-2 p-4 border border-slate-100 rounded bg-slate-50">
            <legend className="text-sm font-semibold">
              Select Systems You Want Access To:
            </legend>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-4 bg-emerald-700 hover:bg-emerald-600 text-white font-semibold py-3 rounded-lg shadow-md cursor-pointer"
          >
